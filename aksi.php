...
if ($mod == 'login') {
    ...
    if ($row) {
        ...
        print_msg("Salah kombinasi user dan password.");
    }
} elseif ($act == 'logout') {
    unset($_SESSION['login']);  // Fixed this line
    ...
    elseif ($pass2 != $pass3)
        ...
        $db->query("UPDATE tb_admin SET pass='$pass2' WHERE username='$_SESSION[login]'");
...
elseif ($mod == 'dataset_tambah') {
    ...
    if ($error) {
        ...
        foreach ($_POST['nilai'] as $key => $val) {
            $db->query("INSERT INTO tb_dataset (nomor, id_atribut, id_nilai, nama) 
                VALUES ('$nomor', '$key', '$val', '$nama')");
        }
    ...
    if ($error) {
        ...
        foreach ($_POST['nilai'] as $key => $val) {
            $db->query("UPDATE tb_dataset SET nama='$nama', id_nilai='$val' WHERE id_dataset='$key'");
        }
        redirect_js("index.php?m=dataset");
    }
...
} else if ($act == 'testing_hapus') {
    $db->query("DELETE FROM tb_testing WHERE nomor='$_GET[ID]'");
    header("location:index.php?m=testing");
}
...
if ($mod == 'atribut_tambah') {
    ...
    $status_atribut = $_POST['status_atribut'];
    $nilai = $_POST['nilai'];
    $keterangan = $_POST['keterangan'];

    if (!$id_atribut || !$nama_atribut || !$status_atribut || $nilai == '')
        print_msg("Field bertanda * tidak boleh kosong!");
    elseif ($db->get_results("SELECT * FROM tb_atribut WHERE id_atribut='$id_atribut'"))
        print_msg("Kode sudah ada!");
    else {
        $db->query("INSERT INTO tb_atribut (id_atribut, nama_atribut, status_atribut, nilai, keterangan) 
            VALUES ('$id_atribut', '$nama_atribut', '$status_atribut', '$nilai', '$keterangan')");
    ...
    if (!$id_atribut || !$nama_atribut || !$status_atribut || $nilai == '')
        ...
        $db->query("UPDATE tb_atribut SET nama_atribut='$nama_atribut', status_atribut='$status_atribut', nilai='$nilai', keterangan='$keterangan' WHERE id_atribut='$_GET[ID]'");
        redirect_js("index.php?m=atribut");
    }
